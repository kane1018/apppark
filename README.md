...
> 本番運用モードです（`siteConfig.noIndex = false` / `isBeta = false`）。
> 公開初期の掲載は、AppPark運営が作成した**実際に動く無料Webツール**（`/tools/...`、「運営作成」表示）です。架空のサービスやデモ数値は表示しません。外部サービスは掲載申請を運営確認のうえ追加します。
> 各種フォーム送信・コメントは、現状サーバー保存なし（仮送信／クライアント保持）です。送信先・保存先を接続すると本運用できます（下記の編集ガイド参照）。閲覧数等の利用実績の数値は、集計基盤が整い `siteConfig.showUsageStats = true` にすると表示されます。
...
サービス投稿・コメント・返信・「役に立った」にはログインが必須です（閲覧は不要）。認証は抽象化されています（`src/components/auth/AuthProvider.tsx`）。

- **環境変数が未設定のあいだ**：メール＋公開表示名の**簡易ログイン**（この端末に保存）で動作します。すぐ全フローを試せます。
- **環境変数を設定すると**：自動で **Supabase Auth**（Google／マジックリンク／メール＋パスワード）に切り替わります。

### Supabaseで本格ログインを有効化する手順
1. [supabase.com](https://supabase.com) でプロジェクトを作成
2. Project Settings → API の URL と anon public key を、Vercel等の環境変数に設定
   - `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`
3. Authentication → Providers で **Google** を有効化（Google CloudでOAuthクライアントを作成）
4. Authentication → URL Configuration の Redirect URLs に `https://<本番URL>/auth/callback` を追加
5. 再デプロイ → Googleログイン等が有効になります（`role` は既定 `user`。管理者は Supabase 側で `app_metadata.role = "admin"` を設定）

### 公開／非公開
- 公開：**公開表示名**・投稿サービス・コメント・プロフィール画像（任意）
- 非公開：メールアドレス・連絡先氏名・管理用メモ・通報者情報
- 通報・削除依頼は既定で「ログイン推奨」（`siteConfig.requireLoginForReport` で必須に切替可）
